...
class ULA_DOA_dataset():
    ...
    def Create_DOA_data_ULA(self, num_signal: int, theta: list, snr=10, snap=512):
        # x(t)=A*s(t)+n(t),先分别生成A,s(t),n(t)
        c = 3 * 10 ** 8  # 光速

        snr = snr  # 信噪比,默认为等功率输入
        snap = snap  # 快拍数
...
if __name__ == '__main__':
    ...
    for delta_theta in delta_thetas:
        ...
        for theta_1 in range(-90, 89 - delta_theta, 1):
            ...
            dataset.Create_DOA_data_ULA(len(theta), theta, snr=10 * np.random.rand(1), snap=512)
        # 用numpy生成,可调用函数更多,更方便
        # for theta_1 in np.arange(-90.0, 89 - delta_theta, step=1):
        #     # 生成角度带有随机性
        #     theta = [theta_1 + np.random.rand(1), theta_1 + delta_theta + np.random.rand(1)]
        #     theta = [float(i) for i in theta]
        #     dataset.Creat_DOA_data_ULA(len(theta), theta, snr=10 * np.random.rand(1), snap=512)
